...
import "./blue.theme.css";                    // atau "./gold.theme.css"


/* ===== Aset khusus classic-blue =====
   Simpan background bunga biru ke: src/assets/classic/blue-bg.png
   (ganti path di bawah kalau kamu letakkan di tempat lain)
*/
import leftImg from "../../../assets/picture.jpg";              // boleh ganti ke foto lain
import rightBg from "../../../assets/background2.png";     // ⬅️ bg biru
...
type InviteStore = {
  ...
  quotes?: string;
  bride?: { ig?: string; fullName?: string; order?: string; father?: string; mother?: string; city?: string; photo?: string; };
  groom?: { ig?: string; fullName?: string; order?: string; father?: string; mother?: string; city?: string; photo?: string; };
...
const getInviteData = (): InviteStore => {
  try { return JSON.parse(localStorage.getItem("invite.data") || "{}"); }
  catch { return {}; }
};

const formatFullDate = (iso?: string, locale = "en-US") =>
  iso
    ? new Date(iso).toLocaleDateString(locale, { weekday: "long", year: "numeric", month: "long", day: "numeric" })
...
function useInviteStore(): InviteStore {
  ...
  useEffect(() => {
    const onStorage = (e: StorageEvent) => {
      if (e.key === "invite.data" || e.key === "invitation.builder") setStore(getInviteData());
    };
...
/* ===================== Main Component (Classic Blue) ===================== */
export default function ClassicBlue() {
  ...
  // scroll helper (lebih akurat di container scroll)
  ...
  return (
    <section className="splitPage theme-classic">
      {/* KIRI */}
      ...
      >
        ...
        {/* Arch SVG: gradasi biru */}
        {/* <svg viewBox="0 0 600 700" className="archSvg" aria-hidden>
          <defs>
            <linearGradient id="blue" x1="0" y1="0" x2="1" y2="1">
              <stop offset="0%"  stopColor="#3c6ba6" />
              <stop offset="50%" stopColor="#789fd6" />
              <stop offset="100%" stopColor="#4f86c6" />
            </linearGradient>
          </defs>
          <path
...
/* ===================== Small Components (sama seperti versi gold) ===================== */
function ScrollCue() {
...
function QuotesCard({ store }: { store: InviteStore }) {
  ...
  try {
    const rawQuotes =
      store?.quotes ?? JSON.parse(localStorage.getItem("invitation.builder") || "{}")?.quotes;
    if (rawQuotes) {
  ...
  return (
    <article className={`profileCard ${variant}`}>
      <div className="avatarWrap">
        {person.photo ? (
          <img className="avatar" src={person.photo} alt={person.fullname} loading="lazy" decoding="async" />
        ) : (
          <div className="avatar avatar--placeholder"><span>IMAGE NOT<br />FOUND</span></div>
        )}
      </div>
      ...
      <p className="parents">{person.father} <br /> {person.mother}</p>
...
function StoryGallerySection({ store }: { store: InviteStore }) {
  ...
  const defaultPics: PicItem[] = [{ src: "/photos/gal1.jpg" }, { src: "/photos/gal2.jpg" }, { src: "/photos/gal3.jpg" }];

  // sumber gambar: store.gallery → /gallery.json → fallback
  useEffect(() => {
    try {
      if (Array.isArray(store?.gallery) && store.gallery.length) {
        setPics(store.gallery.map((it) => (typeof it === "string" ? { src: it } : it)));
        return;
      }
    } catch { }
    fetch("/gallery.json")
      .then((r) => (r.ok ? r.json() : []))
      .then((arr) => {
        if (Array.isArray(arr) && arr.length) setPics(arr.map((it: any) => (typeof it === "string" ? { src: it } : it)));
        else setPics(defaultPics);
      })
  ...
  return (
    <section id="galleries" className="gallerySection story storyPage">
      <div className="galleryHeader">
        ...
        <p className="galleryCaption">"Creating memories is a priceless gift. Memories last a lifetime; objects last only a short time."</p>
      </div>

      <div className="storyWrap">
        <div className="storyRail" ref={railRef}>
          {pics.map((p, i) => (
            <figure className="storyItem" key={i} style={{ background: "transparent" }}>
              <img
                ...
                decoding="async"
                sizes="100vw"
                style={{ background: "#EAF3FB", opacity: loaded[i] ? 1 : 0.001, transition: "opacity .25s ease" }}
                onLoad={() => markLoaded(i)}
              />
              {p.caption && <figcaption className="storyCaption">{p.caption}</figcaption>}
            </figure>
          ))}
        </div>

        <button className="storyNav prev" onClick={() => go(active - 1)} aria-label="Previous">‹</button>
        <button className="storyNav next" onClick={() => go(active + 1)} aria-label="Next">›</button>
      </div>

      <div className="storyDots">
        {pics.map((_, i) => (
          <button key={i} className={i === active ? "is-active" : ""} onClick={() => go(i)} aria-label={`Go to ${i + 1}`} />
        ))}
      </div>
    </section>
  );
...
function DatePages({ store }: { store: InviteStore }) {
  ...
  const akad = {
    ...
    place: store?.akad?.place || "Gereja Katolik Katedral Kristus Raja Purwokerto",
...
function DateCard({ title, dateText, timeText, place, mapsUrl }: { title: string; dateText: string; timeText: string; place: string; mapsUrl: string; }) {
  return (
    <section className="dateSection">
      <div className="dateCard">
        ...
        <a className="dateBtn outline" href={mapsUrl} target="_blank" rel="noreferrer">see location</a>
      </div>
    </section>
  );
}
...
function LocationSection({ store }: { store: InviteStore }) {
  ...
  const embedUrl = `https://www.google.com/maps?q=${place === defaultPlace ? defaultLatLng : encodeURIComponent(place)}&z=17&output=embed`;

  return (
    <section id="location" className="locationSection">
      <div className="mapCard">
        <div className="mapWrap">
          <iframe className="mapFrame" src={embedUrl} loading="lazy" allowFullScreen referrerPolicy="no-referrer-when-downgrade" title="Google Map Location" />
        </div>
        ...
        <div className="dateActions">
          <a className="dateBtn" href={mapsUrl} target="_blank" rel="noreferrer">open in google maps</a>
        </div>
      </div>
    </section>
  );
...
/* ===== RSVP SECTION ===== */
function RsvpSection({ onOpen, store }: { onOpen: () => void; store: InviteStore; }) {
  ...
  return (
    <section id="rsvp" className="rsvpSection">
      <div className="rsvpInner">
        ...
        <p className="rsvpDesc"><b>{count} guest</b> response will join. Please send your response too.</p>
        <button className="rsvpBtn" onClick={onOpen}>Send your response</button>
      </div>
    </section>
  );
}
...
function RsvpModal({ open, onClose }: { open: boolean; onClose: () => void; }) {
  ...
  return (
    <>
      ...
      <aside className="rsvpDrawer" role="dialog" aria-modal="true" aria-label="RSVP Form">
        ...
        <form className="rsvpForm" onSubmit={submit}>
          ...
          <label className="rsvpField">
            ...
            <input value={wa} onChange={(e) => setWa(e.target.value)} required placeholder="6289xxxxxxx" type="tel" inputMode="numeric" />
          </label>
          <label className="rsvpField">
            <span>Address/Company</span>
            <input value={company} onChange={(e) => setCompany(e.target.value)} placeholder="Address/Company" />
          </label>
          <fieldset className="rsvpChoice">
            <legend>Your Response</legend>
            <label><input type="radio" name="resp" value="yes" checked={answer === "yes"} onChange={() => setAnswer("yes")} />Yes, I would like to come</label>
            <label><input type="radio" name="resp" value="no" checked={answer === "no"} onChange={() => setAnswer("no")} />Sorry, I can't</label>
          </fieldset>
          <button className="rsvpSubmit" type="submit">Send your response</button>
        </form>
      </aside>
    </>
  );
  ...
  return (
    <section id="gift" className="panelSection giftSection">
      <div className="panelCard giftCard">
        ...
        <p className="panelSub">Catatan kado atau ucapan terima kasih bisa Anda tulis di bawah ini.</p>
        ...
        <div className="giftMethods">
          ...
          <div className={`giftInfo ${method === "qris" ? "is-active" : ""}`}>
            <label className="giftRadio">
              ...
              <b>QRIS</b>
              <span className="muted">e-wallet (OVO, Gopay, DANA, LinkAja, ShopeePay) & mobile banking</span>
            </label>
            {method === "qris" && (
              <div className="qrisBox">
                {qrisImg ? <img className="qrisImg" src={qrisImg} alt="QRIS" /> : <div className="qrisPlaceholder">QRIS image goes here</div>}
              </div>
            )}
          </div>
              ...
              {banks.map((b, i) => (
                <div key={i} className="bankCard">
                  ...
                  {justCopied === b.accountNumber && (
                    <div className="copiedHint"><Check aria-hidden /> Copied!</div>
                  )}

                  {b.qr && (
                    <>
                      <button type="button" className="qrToggle" onClick={() => setOpenQR(openQR === i ? null : i)}>
                        {openQR === i ? "hide QR" : "show QR"}
                      </button>
                      {openQR === i && (
                        <img className="bankQR" src={b.qr} alt={`QR ${b.bankName}`} loading="lazy" decoding="async" />
                      )}
                    </>
                  )}
                </div>
          ...
          {/* Send Gift */}
          <label className="giftRadio line">
            <input type="radio" checked={method === "send"} onChange={() => setMethod("send")} />
            <b>Send Gift</b>
        ...
        <form className="giftForm" onSubmit={submit}>
          <div className="giftField">
            <span>Your full name</span>
            <input name="name" value={form.name} onChange={onChange} placeholder="Nama lengkap" required />
          </div>
          <div className="giftField">
            <span>Your email address</span>
            <input type="email" name="email" value={form.email} onChange={onChange} placeholder="nama@email.com" required />
          </div>
          <div className="giftField">
            <span>Amount / estimation</span>
            <input name="amount" value={form.amount} onChange={onChange} placeholder="cth: 300.000" inputMode="numeric" />
          </div>
          <div className="giftField">
            <span>Your message</span>
            <textarea name="message" value={form.message} onChange={onChange} rows={3} placeholder="Tulis pesan" />
          </div>
          <button className="giftSubmit" type="submit">make a gift now</button>
        </form>
      </div>
    </section>
  );
...
function WishesSection() {
  ...
  const load = (): Wish[] => {
    try { const s = localStorage.getItem("invite.wishes"); if (s) return JSON.parse(s); }
    catch { }
    return [
      { name: "Dee", city: "Bogor", message: "Beautiful 💞" },
      { name: "Our Wedding Link", city: "Bekasi", message: "Beautiful design matters. happy for all of you that want going to married" },
    ];
  };
  ...
  const initials = (s: string) =>
    (s || "A").trim().split(/\s+/).slice(0, 2).map((v) => v[0]?.toUpperCase()).join("");

  const save = (arr: Wish[]) => localStorage.setItem("invite.wishes", JSON.stringify(arr));

  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    const data: Wish = { name: form.name?.trim() || "Anonymous", city: form.city?.trim(), message: form.message?.trim() || "" };
      ...
      <div className="panelCard wishesCard">
        ...
        <ul className="wishesList" aria-live="polite">
          {wishes.map((w, i) => (
            <li key={i} className="wishItem">
              <div className="wishAvatar" aria-hidden>{initials(w.name)}</div>
        ...
        <form className="wishForm" onSubmit={submit}>
          ...
          <div className="wishField">
            ...
            <textarea name="city" value={form.city} onChange={onChange} placeholder="City / address (optional)" rows={2} />
          </div>
          <div className="wishField">
            <label>Send a wish:</label>
            <textarea name="message" value={form.message} onChange={onChange} placeholder="ex: congrats for this event" rows={3} required />
          </div>
          <button className="wishSubmit" type="submit">submit now</button>
        </form>
      </div>
    </section>
  );
...
function MusicToggle({ className }: { className?: string }) {
  ...
  return createPortal(
    <>
      <audio ref={audioRef} src={musicUrl} loop preload="metadata" playsInline />
      <button
        className={classes}
        onClick={toggle}
