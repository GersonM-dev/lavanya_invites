// src/components/design/modern/modern.tsx
...
/* ===== Aset =====
   - Ganti path sesuai asetmu. Kalau background tidak ada, akan fallback ke gradient (di CSS).
*/
import leftImg from "../../../assets/picture.jpg";
import rightBg from "../../../assets/background3.png";
...
export default function Modern() {
  ...
  // scroll helper agar akurat di container kanan
  ...
  return (
    <section className="splitPage theme-modern">
      {/* KIRI */}
      <div className="split left">
        ...
        <div className="confettiWrap" aria-hidden>
          {confetti.map((c, i) => (
            <span key={i} className="confetti" style={{ top: c.top, left: c.left, animationDelay: c.delay }} />
          ))}
        </div>
      </div>
        ...
        {/* <svg viewBox="0 0 600 700" className="archSvg" aria-hidden>
          <defs>
            <linearGradient id="m-gold" x1="0" y1="0" x2="1" y2="1">
              ...
              <stop offset="50%" stopColor="#F6D56B" />
              <stop offset="100%" stopColor="#C08E3D" />
            </linearGradient>
          </defs>
          <path
            d="M90 540 V170 a210 210 0 0 1 420 0 V540"
            fill="none"
            stroke="url(#m-gold)"
            strokeWidth="6"
          />
          ...
          {pics.map((p, i) => (
            <figure className="storyItem" key={i} style={{ background: "transparent" }}>
              <img
                ...
                decoding="async"
                sizes="100vw"
                style={{ background: "#FAF7F0", opacity: loaded[i] ? 1 : 0.001, transition: "opacity .25s ease" }}
                onLoad={() => markLoaded(i)}
              />
    ...
    <section id="gift" className="panelSection giftSection">
      <div className="panelCard giftCard">
        ...
        <div className="giftMethods">
          ...
          {method === "send" && <div className="addrBox">📦 {shipAddr}</div>}
        </div>
